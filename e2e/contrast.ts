 ...
 *
 *  - text over a *translucent* surface — axe declines to compute a ratio and
 *    files the node under `incomplete`. The hero's "why it matters" aside is
 *    painted `color-mix(in oklab, var(--accent) 6%, transparent)` and the shared
 *    header's buttons `color-mix(in srgb, var(--accent) 38%, transparent)`, so
 *    their text composites onto whatever happens to be behind rather than onto a
 *    flat colour axe can read.
 ...
 *    `opacity: .85` and the `.gloss::after` question mark at `opacity: .9`.
 *
 ...
 * nothing on screen has. The first is live on this page; the other two are kept
 * because they are cheap, silent when unused, and the exact cases a later edit
 * to this lab would reintroduce:
 *
 *  - TEXT CLIPPED AWAY BY AN ANCESTOR PAINTS NOTHING. Content scrolled past a
 *    clipping container's client box is not dimmed or partly drawn — it is
 *    absent from the frame, and asking what colour it sits on has no answer. Its
 *    rect sits outside every ancestor's box, so the walk finds nothing behind it
 *    and falls through to white, inventing a failure.
 *
 ...
 *    five figures are all `<canvas>`, which is opaque to any DOM walk, so the
 *    only SVG here is the shared header's icon set and none of it carries
 *    `<text>`. The rule stands anyway: an `<svg>` draws a shape as a `<rect>`
 *    and then its label as a following `<text>`, so a label's backdrop is a
 *    PRECEDING SIBLING shape, not an ancestor. An ancestor-only walk would
 *    composite those labels onto the svg's own transparent background and report
 *    a ratio nothing on screen has. So for SVG text, any earlier sibling shape
 *    whose box contains the text box is composited first.
 *
 * NOTE ON THE CANVASES. Everything this lab draws — the isogeny map, the graph,
 * the key exchange, the key space — is painted into a `<canvas>`. No DOM oracle
 * can see inside one. Those colours are measured by hand from screenshot pixels
 * and their findings fixed in `src/style.css`; this file cannot cover them and
 * does not pretend to.
 */
...
export async function auditContrast(page: Page): Promise<ContrastFailure[]> {
  return page.evaluate(() => {
     ...
     * accent into surfaces with `color-mix()` in both `oklab` and `srgb` (the
     * hero's "why it matters" wash is `color-mix(in oklab, var(--accent) 6%,
     * transparent)`, the shared header's borders `color-mix(in srgb, ...)`), and
     ...
     * fabricates failures (a dark-on-light panel read as dark-on-dark) and, far
     ...
     * wherever the text sits. That is right for a gradient that spans its
     * element and badly wrong for a page-sized wash: a decorative glow in one
     * corner would then be used as the backdrop for a footer a full page-height
     * below, inventing a surface that text never sits on. So each gradient is
     * *sampled* at the text's real location: linear by projecting onto the
     * gradient line, radial by distance from the centre over the farthest-corner
     * radius. This lab's stylesheet paints no gradients today — every surface is
     * a flat token or a `color-mix()` of one — so this path is currently inert,
     * and it is kept so that adding one cannot silently go unmeasured. A
     ...
     *
     * A full drive of this page leaves five exhibits' worth of `.kv` result rows
     * on screen — two text nodes each — plus the glossary popover and the model
     * notes, and every one of them walks the same handful of ancestors up to
     * `<section>`, `<main>` and `<body>`. Without the caches the pass re-reads
     * the same computed styles and rects thousands of times. Nothing mutates the
     ...
     * this fleet at 1.15:1 for a card's screen-reader-only label. This lab uses
     * the off-screen-parking variant of the idiom rather than the clip one (see
     * the skip-link note below), so this guard is currently inert here; it stays
     * because the two spellings are interchangeable and swapping to `clip` must
     * not start manufacturing failures. Deliberately narrow — only a ZERO-area
     * clip qualifies, so a real partial clip is still measured.
     */
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // `content-visibility: hidden` and a closed `<details>` hide a subtree
      // without `display: none`, and Chromium keeps the last laid-out geometry
      // for it — so the `display`/rect tests above all pass for text that paints
      // nothing. This lab has no disclosure widget today; the glossary popover
      // hides with the `hidden` attribute, which the `display` test above
      // already catches.
      ...
      // slides to `top: 0` only on focus. Measuring the parked copy invents a
     ...
     * catch what axe *misses* among exposed text (translucency, opacity), not to
     * be stricter than axe on decorative content. Honour the same boundary.
     * On this page the only `aria-hidden` subtrees are the shared header's
     * `<svg>` icons, which own no text node, so the rule costs nothing here. It
     * is kept because the alternative — being stricter than axe on content that
     * is deliberately outside the accessibility tree — produces findings with no
     * corresponding user-visible failure, which is how a gate loses its
     * credibility.
     */
